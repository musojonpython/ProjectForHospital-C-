...
void __fastcall TForm_Bolim_boshligi_uchun_Bemorlar::LabeledEdit1Change(TObject *Sender)
{
 ...
 while(!ADOQuery2->Eof)
 {
	 if (ADOQuery2->FieldByName("Familiya")->AsString == fam) {
		i_d = ADOQuery2->FieldByName("i_d")->AsString;
	 }
	  ADOQuery2->Next();
 }
 ADOQuery1->Close();
 ...
 ADOQuery1->SQL->Add("and Ismi LIKE '" + LabeledEdit2->Text + "%'");
 ADOQuery1->SQL->Add("and Bolim_id = " + Shikorlar_Login_va_Parol->Edit3->Text);
 ADOQuery1->SQL->Add("and Shifokor_id = " + i_d);
 ADOQuery1->Open();
 }
...

{
	...
	Form_Malumotqoshish->Bolim_boshligi_uchun->Checked = true;
}
//---------------------------------------------------------------------------
...

{
   ...
   ADOQuery1->First();
   while(!ADOQuery1->Eof){
	if(Shikorlar_Login_va_Parol->Edit3->Text == ADOQuery1->FieldByName("Bolim_id")->AsString)
	 s++;
	 ADOQuery1->Next();
  }
...

{
   ...
   while(!ADOQuery2->Eof)
   {
	   ...
	   {
		 id_shifokor =  ADOQuery2->FieldByName("i_d")->AsInteger;
		 break;
	   }
		   ADOQuery2->Next();
   }
   ...
   ADOQuery1->SQL->Clear();
   ADOQuery1->SQL->Add("select * from Bemorlar_haqida");
   ADOQuery1->SQL->Add("where shifokor_id = " + IntToStr(id_shifokor));
   ADOQuery1->SQL->Add(" and Bolim_id = " + Shikorlar_Login_va_Parol->Edit3->Text);
   ADOQuery1->Open();
   }
  ...
  s = 0;
   }else
   {
		ADOQuery1->First();
   while(!ADOQuery1->Eof){
	if(Shikorlar_Login_va_Parol->Edit3->Text == ADOQuery1->FieldByName("Bolim_id")->AsString && ADOQuery1->FieldByName("shifokor_id")->AsString == IntToStr(id_shifokor))
	 s++;
	 ADOQuery1->Next();
  }
  Bemorlar_soni->Caption = IntToStr(s);
...
{
	  Form_Bolim_boshligi_uchun_Bemorlar->Close();
	  Form_Shifokorlar_bolim_boshligi_uchun->Caption = "Bo'lim boshlig'ining shifokorlar ro'yxati";
	  ...
	  Form_Shifokorlar_bolim_boshligi_uchun->ADOQuery1->SQL->Add("where bolim_i_d = " + Shikorlar_Login_va_Parol->Edit3->Text);
